...
even_row_style <- createStyle(fontSize = 12, fgFill = "#63B8FF") #B9DDF1
...
create_table_no_k(wb, "Report_Loans", r.type.gbv, "Loans by Type", 3, 14) #5 rows
create_table_no_k(wb, "Report_Loans", r.status.gbv, "Loans by Status", 3, 23) #4 rows
#create_table_no_k(wb, "Report_Loans", Profile_Numeric, "Profile Numeric", 3, 31) #8 rows

ggsave("Charts/gbv_residual_%_loan_type.png",plot = chart.gbv.perc)
insertImage(wb,sheet = "Report_Loans","Charts/gbv_residual_%_loan_type.png",startCol = 13, startRow = 8, width = 6, height = 4.5, dpi = 300)
ggsave("Charts/gbv_residual_%_loan_type.png",plot = chart.date.status.cases)
insertImage(wb,sheet = "Report_Loans","Charts/gbv_residual_%_loan_type.png",startCol = 13, startRow = 8, width = 6, height = 4.5, dpi = 300)
...
#apply the functions as follows:   workbook, "sheetname", dataframe, "table title", ncol, nrow
#create_table_k_for_miles(wb, "Report", r.introductionP6, "Summary", 3, 5)
create_table_no_k(wb, "Report_Borrowers", r.borrower.n.entities, "Borrowers by N Entities", 3, 5) #5 rows
create_table_no_k(wb, "Report_Borrowers", r.n.borrowers.n.loans, "Borrowers by N Loans", 3, 13) #7 rows
create_table_no_k(wb, "Report_Borrowers", r.type.subject.n.cases, "Borrowers by Type", 3, 23) #4 rows
create_table_no_k(wb, "Report_Borrowers", r.individuals.range.age, "Borrowers by Age Range", 3, 30) #7 rows
